...
output_directory=os.path.join(current_directory,'..','temp_folder_for_results','Vault_analyzer')
...
class GUI:
    ...
    def main_frame (self, root):
        def select_path ():
            ...
            #Include within the variable 
            self.arch_parameters["path_data"]=path
            
        root.title ("Analysis of vaults")
        ...
        # Labels            
        label_pc = tk.Label(root, text="Choose point cloud:")
        label_pc.grid(row=0, column=0, sticky=tk.W)
        
        label_vault = tk.Label(root, text="Type of vault:")
        label_vault.grid(row=1, column=0, sticky=tk.W)
        
        label_step = tk.Label(root, text="Space between sections:")
        label_step.grid(row=2, column=0, sticky=tk.W)    
        
        label_thickness = tk.Label(root, text="Tickness of each section:")
        label_thickness.grid(row=3, column=0, sticky=tk.W)
        
        label_zmax = tk.Label(root, text="Maximum z value for extracting the axis:")
        label_zmax.grid(row=4, column=0, sticky=tk.W) 
        
        label_zmin = tk.Label(root, text="Minimum z value for extracting the axis:")
        label_zmin.grid(row=5, column=0, sticky=tk.W)
        
        label_s = tk.Label(root, text="Smoothing parameter for the axis extraction:")
        label_s.grid(row=6, column=0, sticky=tk.W)

        label_e = tk.Label(root, text="Expected clearance of the vault:")
        label_e.grid(row=7, column=0, sticky=tk.W)  
        
        save_path_label = tk.Label(root, text="Path for saving the data:")
        save_path_label.grid(row=9, column=0, sticky="w",pady=2)
        
        label_setup = tk.Label(root, text="Configuration of the arch estimator:")
        label_setup.grid(row=8, column=0, sticky=tk.W) 
        
        # Combobox
        combo1=ttk.Combobox (root,values=name_list)
        combo1.grid(row=0,column=1, sticky="e", pady=2)
        combo1.set("Not selected")
        
        types = ["Barrel vault"]
        combo2=ttk.Combobox (root,values=types, state="readonly")
        combo2.current(0)
        combo2.grid(column=1, row=1, sticky="e", pady=2)
        combo2.set("Not selected")
        
        # Entry
        entry_step = tk.Entry(root,width=5)
        entry_step.grid(row=2,column=1, sticky="e",pady=2)
        entry_step.insert(1,self.vault_parameters["step"])
        
        entry_thickness = tk.Entry(root,width=5)
        entry_thickness.grid(row=3,column=1, sticky="e",pady=2)
        entry_thickness.insert(0,self.vault_parameters["thickness"])
        
        entry_zmax = tk.Entry(root,width=5)
        entry_zmax.grid(row=4,column=1, sticky="e",pady=2)
        entry_zmax.insert(0,self.vault_parameters["zmax"])
        
        entry_zmin = tk.Entry(root,width=5)
        entry_zmin.grid(row=5,column=1, sticky="e",pady=2)
        entry_zmin.insert(0,self.vault_parameters["zmin"])  
        
        entry_s = tk.Entry(root,width=5)
        entry_s.grid(row=6,column=1, sticky="e",pady=2)
        entry_s.insert(0,self.vault_parameters["s"])  
        
        entry_e = tk.Entry(root,width=5)
        entry_e.grid(row=7,column=1, sticky="e",pady=2)
        entry_e.insert(0,self.vault_parameters["e"])      

        # Textbox
        save_path_textbox = tk.Entry(root,width=30)
        save_path_textbox.grid(row=9,column=1, sticky="e",pady=2)
        save_path_textbox.insert(0, output_directory)

        # Button
        setup_button= ttk.Button (root, text="...", command=lambda: self.show_set_up_window(),width=10)
        setup_button.grid (row=8,column=2,sticky="e",padx=100)
        save_path_button = ttk.Button(root, text="...", command=select_path,width=10)
        save_path_button.grid(row=9, column=2, sticky="e",padx=100)
        run_button= ttk.Button (root, text="OK", command=lambda:self.run_algorithm_1(combo1.get(),combo2.get(),float(entry_step.get()),float(entry_thickness.get()),float(entry_zmax.get()),float(entry_zmin.get()),float(entry_e.get()),float(entry_s.get())), width=10)
        run_button.grid (row=10,column=1,sticky="e",padx=100)
        cancel_button= ttk.Button (root, text="Cancel", command=self.destroy_1,width=10)
        cancel_button.grid (row=10,column=1,sticky="e") 
    ...
    def show_set_up_window (self):      
        ...
        label_thickness=tk.Label(set_up_window, text="Thickness threshold:")
        label_thickness.grid(row=0, column=0, sticky="w",pady=2)
        
        label_degree=tk.Label(set_up_window, text="Type of arch:")
        label_degree.grid(row=1, column=0, sticky="w",pady=2)

        label_threshold_ransac=tk.Label(set_up_window, text="Threshold value for RANSAC fitting:")
        label_threshold_ransac.grid(row=2, column=0, sticky="w",pady=2)

        label_iterations_ransac=tk.Label(set_up_window, text="Number of iteration for RANSAC fitting:")
        label_iterations_ransac.grid(row=3, column=0, sticky="w",pady=2)

        label_min_samples=tk.Label(set_up_window, text="Minimum number of samples for fitting the model:")
        label_min_samples.grid(row=4, column=0, sticky="w",pady=2)

        label_fix=tk.Label(set_up_window, text="Fixed springing line:")
        ...
        checkbox1_label = tk.Label(set_up_window, text="Load the points of the main section")
