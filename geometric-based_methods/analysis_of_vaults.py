...
class GUI_vaults(tk.Frame):
    ...
    def main_frame (self, window):
        ...
        # run_button= ttk.Button (window, text="OK", command=lambda:self.run_algorithm_1(combo1.get(),combo2.get(),float(entry_step.get()),float(entry_thickness.get()),float(entry_zmax.get()),float(entry_zmin.get()),float(entry_e.get()),float(entry_s.get())), width=10)
        # run_button.grid (row=10,column=1,sticky="e",padx=100)
        # cancel_button= ttk.Button (window, text="Cancel", command=self.destroy_1,width=10)
